require File.dirname(__FILE__) + '/cw_pagination_action_controller.rb'
module CW
module Pagination
...
# 	Nothing (nil)
	...
	def items_on_page( page_number )
		raise ArgumentError.new( "Page number: #{page_number}, is out of range." ) unless valid_page_number?( page_number )
		# set the page number to 0-based index, then multiply by items_per_page
		offset = ( page_number - 1 ) * items_per_page

		item_class.find( :all, self.query.merge( { :limit => items_per_page, :offset => offset, :order => ordering_sql_string } ) )
	end
...
#		(int) The position of the object using a 1-based index
	def item_number( object )
		...
		uq = query.dup
		uq[:conditions] = [''] unless uq.has_key?(:conditions) # Fix if missing
		uq[:conditions] = [uq[:conditions]] if uq[:conditions].is_a?(String) # Fix if not an array

		nq = item_number_conditions( object )
		if nq.has_key?(:conditions)
			s = nq[:conditions].shift
			# Tack on the conditions, or just set our conditions
			if uq[:conditions].first.strip.empty?
				uq[:conditions][0] = s
			else
				uq[:conditions][0] = '(' + uq[:conditions][0] + ') AND ' + s
			end

			# append the values
			uq[:conditions].concat( nq[:conditions] )
		end

		# Use Active Record to count all items before the one we want
		item_class.count( uq.reject{|k,v| k.eql?(:select) or k.eql?(:limit) or k.eql?(:offset) } )
...
# Now, the rest of the clauses are built to include all previous clauses
	...
	def absolute_column_name( column )
		if column.include?('.')
			"#{column}"
		else
			"#{item_class.table_name}.#{column}"
		end
	end
	protected :absolute_column_name


	def count_items
		item_class.count( @query.reject{|k,v| k.eql?(:select) or k.eql?(:limit) or k.eql?(:offset) } )
	end
