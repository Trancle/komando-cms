...
# @returns the current page number starting from index 1 to count_pages
	def current_page_number
		...
		page_param_v = @controller.params[page_number_param_name]
		if page_param_v.nil? or ( page_param_v.is_a?(String) and ( page_param_v.empty? or page_param_v.to_i.eql?(0) ) )
...
The following method generates a hash with keys and values that are to be preserved between the pagination jumps. Note, this does NOT include the page key as it's assumed that you'll not be using the current page to do jumps. If you really want it, use the parameters_with_current_page method

Returns:
	A hash containing the preserved parameter names and values of those preserved parameters
=end
	def parameters
		r = {}
		self.preserve_parameters.each{|pp| r[pp] = self.controller.params[pp] }
		r
	end

=begin
Parameters With Current Page

Same as Parameters method above, but also includes the current page

Arguments:
	controller: (ActionController) to use to lookup the parameters to preserve

Returns:
	A hash containing the preserved parameter names and values of those preserved parameters
=end
	def parameters_with_current_page
		self.parameters.merge({ self.page_number_param_name => current_page_number })
	end
